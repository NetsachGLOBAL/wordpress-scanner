...
def get_interesting_headers(url):
    try:
        response = requests.get(url)
        headers = response.headers
        interesting_headers = {key: value for key, value in headers.items() if key.lower() in ["x-frame-options", "x-content-type-options", "x-xss-protection", "strict-transport-security"]}
